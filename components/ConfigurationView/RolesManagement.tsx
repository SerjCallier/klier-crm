...
import { 
  ...
  Clock, ArrowRight, History, Info, AlertTriangle
} from 'lucide-react';
import { ALL_USERS, ROLES, PERMISSIONS } from '../../constants';
import { User, Role, UserStatus, UserRole, Permission, AuditLog } from '../../types';
import { useAuth } from '../../AuthContext';

// Mock de logs para simular la auditoría por usuario
const MOCK_AUDIT_LOGS: AuditLog[] = [
  { id: '1', userId: 'u1', userName: 'Sergio Callier', action: 'CREATE_LEAD', module: 'leads', timestamp: new Date(Date.now() - 3600000).toISOString(), status: 'success' },
  { id: '2', userId: 'u1', userName: 'Sergio Callier', action: 'UPDATE_ROLE', module: 'users', timestamp: new Date(Date.now() - 86400000).toISOString(), status: 'success' },
  { id: '3', userId: 'u2', userName: 'Ventas 1', action: 'LOGIN_SUCCESS', module: 'auth', timestamp: new Date(Date.now() - 1200000).toISOString(), status: 'success' },
  { id: '4', userId: 'u2', userName: 'Ventas 1', action: 'UPDATE_LEAD', module: 'leads', entityId: 'l1', timestamp: new Date(Date.now() - 5400000).toISOString(), status: 'success' },
  { id: '5', userId: 'u3', userName: 'UX/UI 1', action: 'DELETE_TASK', module: 'tasks', timestamp: new Date(Date.now() - 172800000).toISOString(), status: 'success' },
];
...
export const RolesManagement: React.FC = () => {
  ...
  const [auditModal, setAuditModal] = useState<{ isOpen: boolean; user: User | null }>({ isOpen: false, user: null });
  ...
  const userLogs = useMemo(() => {
    ...
    return MOCK_AUDIT_LOGS.filter(log => log.userId === auditModal.user?.id)
  ...
  const getActionIcon = (action: string) => {
    if (action.includes('CREATE')) return <Plus size={12} className="text-green-500" />;
    if (action.includes('UPDATE')) return <Edit2 size={12} className="text-blue-500" />;
    if (action.includes('DELETE')) return <Trash2 size={12} className="text-red-500" />;
    if (action.includes('LOGIN')) return <Key size={12} className="text-purple-500" />;
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors p-6 overflow-y-auto">
                    ...
                    <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button 
                              onClick={() => setAuditModal({ isOpen: true, user: u })}
                            ...
                            {canManage && u.id !== authUser.id && (
                                <button 
                                  ...
                                  className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-blue-900/30 rounded-lg transition-all"
      ...
      {/* User Audit Modal */}
      {auditModal.isOpen && (
          <div className="fixed inset-0 bg-black/70 z-[110] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
              <div className="bg-white dark:bg-slate-850 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700 flex flex-col max-h-[85vh]">
                  <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-slate-800 dark:text-white leading-tight">Auditoría de Actividad</h3>
                            <p className="text-xs text-slate-500 font-medium">{auditModal.user?.name} ({auditModal.user?.email})</p>
                        </div>
                      </div>
                      <button onClick={() => setAuditModal({ isOpen: false, user: null })} className="text-slate-400 hover:text-red-500 transition-all p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
                          <X size={24} />
                      </button>
                  </div>

                  <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-white dark:bg-slate-850">
                      {userLogs.length > 0 ? (
                        <div className="relative border-l-2 border-slate-100 dark:border-slate-800 ml-3 space-y-8 pl-8 py-2">
                           {userLogs.map((log) => (
                             <div key={log.id} className="relative group">
                                <div className="absolute -left-[41px] w-5 h-5 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center shadow-sm z-10">
                                ...
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border ${log.status === 'success' ? 'bg-green-50 text-green-600 border-green-100 dark:bg-green-900/20 dark:border-green-800/40' : 'bg-red-50 text-red-600 border-red-100 dark:bg-red-900/20 dark:border-red-800/40'}`}>
                                        {log.status}
                                    </span>
                                </div>
                                
                                <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm group-hover:border-blue-200 dark:group-hover:border-blue-900/50 transition-all">
                                    <div className="flex items-center gap-2 mb-1.5">
                                        <span className="text-xs font-bold text-slate-800 dark:text-white">{log.action.replace(/_/g, ' ')}</span>
                                        <span className="text-[9px] text-blue-500 font-bold uppercase tracking-tighter">en {log.module}</span>
                                    </div>
                                    {log.entityId && (
                                        <p className="text-[10px] text-slate-500 flex items-center gap-1">
                                            <Info size={10} /> ID Objeto: <span className="font-mono text-slate-400">{log.entityId}</span>
                                        </p>
                                    )}
                                </div>
                             </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-slate-400 opacity-50">
                            ...
                            <p className="text-sm font-medium">Sin registros de actividad para este usuario</p>
                        </div>
                      )}
                  </div>
                  ...
                  <div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
                      <button 
                        onClick={() => setAuditModal({ isOpen: false, user: null })}
                        className="w-full py-2.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all uppercase tracking-widest text-xs"
                      >
                        Cerrar Auditoría
                      </button>
                  </div>
