...
import { Target, Lightbulb, Calendar, ArrowRight, CheckCircle, Circle, Archive, Plus, RefreshCw, Trash2, Clock, Infinity, Layers, Zap, TrendingUp, Users, ArrowUpRight, Sparkles } from 'lucide-react';
...
const PERPETUAL_STAGES: PerpetualStage[] = [
    {
        ...
        subtitle: 'RESCATE INMEDIATO (SLA 24h)',
        icon: Zap,
...
export const StrategicView: React.FC = () => {
    ...
    const handleAISuggestion = async () => {
        ...
        const context = `Estamos en el Centro de Operaciones (Plan de Batalla). El modelo es "Digital Rescue" con entregas en 48h. Las etapas son: 1. Rescate Inmediato (SAME-DAY), 2. Infraestructura Crítica (Ecommerce), 3. Motor Perpetuo (SEO Local), 4. Automatización Táctica (WhatsApp/IA). Sugiere 3 maniobras estratégicas accionables.`;

        try {
            const response = await generateAIResponse(
                "Genera una lista de 3 maniobras de rescate estratégico cortas y potentes para un equipo de respuesta rápida.",
                AIMode.FAST,
                context
            );

            // Basic parsing (expecting bullet points or numbered list)
            ...
            suggestions.forEach(text => {
                const newItem: StrategyItem = {
                    ...
                    stageId: 'stage_1' // Default to Impact
                };
                setStrategies(prev => ({
                    ...prev,
                    landing: [...(prev.landing || []), newItem]
                }));
            });
        } catch (e) {
            console.error(e);
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-2 uppercase tracking-tight">
                            <Target className="text-blue-600 dark:text-blue-400" /> Plan de Batalla
                        </h2>
                    ...
                    <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-lg">
                        ...
                        <div className="w-px h-6 bg-slate-200 dark:bg-slate-600 self-center mx-1" />
                        <button
                            onClick={handleAISuggestion}
                            disabled={isGenerating}
                            className="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-sm transition-all disabled:opacity-50"
                        >
                    ...
                    <div className="flex gap-6 overflow-x-auto no-scrollbar">
                        {MOCK_NICHES.map(niche => (
                            <button
                                ...
                                className={`pb-3 px-1 text-sm font-medium border-b-2 transition-colors whitespace-nowrap flex flex-col items-start ${activeNicheId === niche.id
                ...
                {viewMode === 'cycle' && (
                    <div className="h-full overflow-y-auto custom-scrollbar">
                        ...
                        <div className="mt-8 p-6 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30">
                            <h3 className="text-lg font-black text-blue-800 dark:text-blue-300 mb-2 flex items-center gap-2 uppercase tracking-tight">
                                <Infinity size={20} /> Estrategia de Rescate 48h / Marketing Perpetuo
                            </h3>
                            <p className="text-sm text-blue-700 dark:text-blue-400 leading-relaxed max-w-4xl font-medium">
                                En KlierNav, no solo reparamos webs; ejecutamos un <strong>Digital Rescue</strong>. Nuestra misión es la excelencia en 48 horas: un cliente entra por un 
                                <strong> Rescate Inmediato</strong> (Etapa 1), se estabiliza con <strong>Infraestructura Crítica</strong> (Etapa 2), 
                                se expande con el <strong>Motor Perpetuo</strong> (Etapa 3) y se domina el nicho con 
                                <strong> Automatización Táctica</strong> (Etapa 4). Este plan de batalla es tu mapa para el éxito asegurado.
                            </p>
                        </div>
                    </div>
                )}
