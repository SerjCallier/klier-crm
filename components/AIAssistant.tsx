...
export const AIAssistant: React.FC<AIAssistantProps> = ({ leads }) => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'model',
      text: 'Hola Sergio, soy el Coordinador de Operación de KlierNav. Despliegue de "Digital Rescue" activo. ¿En qué misión priorizamos hoy? Puedo analizar leads, optimizar rescates o coordinar entregas 48h.'
    }
  ]);
  ...
  const getSuggestions = () => {
    switch (mode) {
        case AIMode.SEARCH:
            return [
                "Inteligencia: Competencia de TechStore",
                "Exploración: Tendencias E-commerce Argentina 2025",
                "Táctica: Mejores prácticas SEO local",
                "Logística: Nuevas automatizaciones WhatsApp"
            ];
        case AIMode.REASONING:
            return [
                "Maniobra: Upsell para rescates finalizados",
                "Análisis: Rentabilidad Digital Rescue 48h",
                "Logística: Optimizar tiempos de despliegue",
                "Táctica: Optimizar embudo de respuesta rápida"
            ];
        case AIMode.FAST:
        default:
            return [
                "Redactar: Propuesta de Rescate Inmediato",
                "Reporte: Misiones críticas pendientes",
                "Cálculo: Presupuesto E-commerce Express",
                "Email: Seguimiento Táctico Inmobiliaria"
            ];
    }
  };
