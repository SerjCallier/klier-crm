...
export const TasksView: React.FC<TasksViewProps> = ({ tasks, leads, onAddTask, onToggleTask, onDeleteTask }) => {
  ...
  const fetchAISuggestions = async () => {
    ...
    const prompt = `Como analista táctico de KlierNav (Operation Center), revisa estos leads y genera 4 misiones críticas de "Digital Rescue" para hoy. Usa lenguaje de acción y urgencia. Devuélvelas separadas por comas únicamente. Sin números.`;
  ...
  const getPriorityBadge = (priority: TaskPriority) => {
      switch(priority) {
          case 'high':
              return (
                   <span className="flex items-center gap-1 text-[10px] font-black bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full border border-red-200 dark:border-red-800 uppercase tracking-tighter">
                       <Flag size={10} fill="currentColor" /> CRÍTICA
                   </span>
              );
          case 'medium':
              return (
                   <span className="flex items-center gap-1 text-[10px] font-bold bg-orange-50 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400 px-2 py-0.5 rounded-full border border-orange-100 dark:border-orange-800 uppercase tracking-tighter">
                       <Flag size={10} /> ESTÁNDAR
                   </span>
              );
          case 'low':
              return (
                   <span className="flex items-center gap-1 text-[10px] font-bold bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-600 uppercase tracking-tighter">
                       <Flag size={10} /> SOPORTE
                   </span>
              );
      }
  };
  ...
  return (
    <div className="p-6 h-full overflow-y-auto bg-slate-50 dark:bg-slate-900 transition-colors relative">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Misiones Operativas</h2>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="bg-[#D32F2F] hover:bg-[#B71C1C] text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-black uppercase tracking-widest transition-all shadow-lg shadow-red-500/20 active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
            Pendientes <span className="bg-[#D32F2F] text-white text-[10px] px-2 py-0.5 rounded-full font-black animate-pulse">{pendingTasks.length}</span>
          </h3>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in transition-colors">
            <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-750">
              <h3 className="font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                <Plus size={18} /> Planificar Misión de Rescate
              </h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-black text-slate-700 dark:text-slate-300 mb-1 uppercase tracking-wider text-[10px]">Objetivo de la Misión</label>
                <input 
                  type="text"
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 px-4 py-2 bg-[#D32F2F] text-white rounded-lg font-black uppercase tracking-widest text-xs hover:bg-[#B71C1C] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-red-500/10 active:scale-95"
                >
                  Confirmar Misión
                </button>
              </div>
