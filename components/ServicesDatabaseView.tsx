...
import { 
  ...
  Copy, Check, MessageSquare, Send
} from 'lucide-react';
import { INITIAL_SERVICES, SERVICE_HEX, SERVICE_COLORS, MESSAGE_TEMPLATES } from '../constants';
...
export const ServicesDatabaseView: React.FC = () => {
  ...
  const [templateViewService, setTemplateViewService] = useState<Service | null>(null);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors p-6 overflow-hidden">
          ...
          {filteredServices.map((service) => (
            ...
            >
              ...
              <div className="px-6 py-4 bg-slate-50 dark:bg-slate-750 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
                  ...
                  <button 
                    onClick={() => setTemplateViewService(service)}
                    className={`text-xs font-bold flex items-center gap-1 hover:underline ${service.isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 cursor-not-allowed'}`}
      ...
      {/* Templates Modal */}
      {templateViewService && (
          <div className="fixed inset-0 bg-black/60 z-[110] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
              <div className="bg-white dark:bg-slate-850 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700 flex flex-col max-h-[85vh]">
                  <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-2xl ${SERVICE_COLORS[templateViewService.type]}`}>
                            <MessageSquare size={24} />
                        </div>
                        <div>
                            <h3 className="font-bold text-slate-800 dark:text-white leading-tight">Plantillas de Venta</h3>
                            <p className="text-xs text-slate-500 font-medium">{templateViewService.name}</p>
                        </div>
                      </div>
                      <button onClick={() => setTemplateViewService(null)} className="text-slate-400 hover:text-red-500 transition-all p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
                          <X size={24} />
                      </button>
                  </div>

                  <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6 bg-slate-50/30 dark:bg-slate-900/30">
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl border border-blue-100 dark:border-blue-800/40 flex gap-3 items-start">
                          <Info className="text-blue-500 shrink-0 mt-0.5" size={16} />
                          <p className="text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed">
                              Usa estas respuestas para agilizar el contacto inicial. Haz clic en <strong>"Copiar"</strong> para llevar el texto a tu WhatsApp.
                          </p>
                      </div>

                      <div className="space-y-4">
                        {(MESSAGE_TEMPLATES[templateViewService.type] || MESSAGE_TEMPLATES['Other']).map((text, idx) => (
                            <div key={idx} className="group relative">
                                <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:border-blue-300 dark:hover:border-blue-800 transition-all">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Variante #{idx + 1}</span>
                                        <button 
                                            onClick={() => handleCopyTemplate(text, idx)}
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all ${copiedIndex === idx ? 'bg-green-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-blue-600 hover:text-white'}`}
                                        >
                                            {copiedIndex === idx ? <><Check size={12} /> Copiado</> : <><Copy size={12} /> Copiar</>}
                                        </button>
                                    </div>
                                    <div className="bg-[#E7FFDB] dark:bg-[#005c4b] p-3 rounded-xl rounded-tl-none border border-[#d1f4cc]/50 dark:border-white/5 shadow-sm relative ml-2">
                                        <div className="absolute top-0 -left-2 w-0 h-0 border-t-[10px] border-t-[#E7FFDB] dark:border-t-[#005c4b] border-l-[10px] border-l-transparent"></div>
                  ...
                  <div className="p-6 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
                      <button 
                        onClick={() => setTemplateViewService(null)}
                        className="w-full py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-2xl hover:bg-slate-200 transition-all uppercase tracking-widest text-xs"
                      >
                        Cerrar Explorador
                      </button>
                  </div>
