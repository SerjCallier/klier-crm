...
import { LayoutDashboard, Kanban, Calendar as CalendarIcon, MessageCircle, Settings, Menu, UserCircle, CheckSquare, Moon, Sun, Sparkles, Target, History, Shield, Box, HelpCircle } from 'lucide-react';
      ...
      <div className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col transition-all duration-300 shadow-sm z-20`}>
        <div className="p-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-700 h-16 transition-colors">
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {isSidebarOpen && <span className="font-bold text-lg text-slate-800 dark:text-white whitespace-nowrap">KlierNav - Operation Center</span>}
          </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto custom-scrollbar">
          <div className="pb-2 px-4">
            {isSidebarOpen ? <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Operaciones</p> : <div className="h-px bg-slate-100 dark:bg-slate-700 mb-2" />}
          </div>
          <NavItem view={View.DASHBOARD} icon={LayoutDashboard} label="Resumen Táctico" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          <NavItem view={View.PIPELINE} icon={Kanban} label="Pipeline de Rescate" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          <NavItem view={View.MESSAGING} icon={MessageCircle} label="Comunicaciones" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          <NavItem view={View.TASKS} icon={CheckSquare} label="Misiones" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          <NavItem view={View.CALENDAR} icon={CalendarIcon} label="Agenda de Despliegue" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          <NavItem view={View.SERVICES} icon={Box} label="Catálogo" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />

          <div className="pt-4 pb-2 px-4">
            {isSidebarOpen ? <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Estrategia & IA</p> : <div className="h-px bg-slate-100 dark:bg-slate-700 my-2" />}
          </div>

          {canAccessModule(user, 'strategy') && (
            <NavItem view={View.STRATEGY} icon={Target} label="Plan de Batalla" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
          )}
          <NavItem view={View.AI_CHAT} icon={Sparkles} label="Inteligencia IA" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />

          <div className="pt-4 pb-2 px-4">
            {isSidebarOpen ? <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Configuración</p> : <div className="h-px bg-slate-100 dark:bg-slate-700 my-2" />}
          </div>
          ...
          <NavItem view={View.MANUAL} icon={HelpCircle} label="Manual de Usuario" currentView={currentView} setCurrentView={setCurrentView} isSidebarOpen={isSidebarOpen} />
        </nav>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
        <header className="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-10 transition-colors">
          <h1 className="text-xl font-semibold text-slate-800 dark:text-white">
            {currentView === View.DASHBOARD && 'Resumen Táctico'}
            {currentView === View.STRATEGY && 'Planificación Táctica (Plan de Batalla)'}
            {currentView === View.LEADS_LIST && 'Directorio de Rescate'}
            {currentView === View.SERVICES && 'Arsenal de Soluciones Digitales'}
            {currentView === View.PIPELINE && 'Gestión de Operaciones de Rescate'}
            {currentView === View.MESSAGING && 'Comunicaciones con Prospectos'}
            {currentView === View.TASKS && 'Gestión de Misiones Críticas'}
            {currentView === View.CALENDAR && 'Agenda y Ventana de Despliegue'}
            {currentView === View.AI_CHAT && 'Centro de Inteligencia de Mercado'}
            {currentView === View.USERS_ROLES && 'Control de Agentes'}
            {currentView === View.AUDIT_LOGS && 'Registro de Actividad Táctica'}
            {currentView === View.MANUAL && 'Manual Operativo'}
